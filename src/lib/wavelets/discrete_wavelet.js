...
import * as wfs from './waveletCoefficients'
function swap(a, b) {
  let tmp = b
  b = a
  a = tmp
...
function discrete_wavelet(name: WAVELET_NAME, order: number): DiscreteWavelet {
  ...
  switch (name) {
    ...
    case 'RBIO':
      ...
      swap(w.rec_lo, w.dec_lo)
      swap(w.rec_hi, w.dec_hi)
      for (let i = 0, j = w.rec_len - 1; i < j; i++, j--) {
        [w.rec_lo[i], w.rec_lo[j]] = [w.rec_lo[j], w.rec_lo[i]];
        [w.rec_hi[i], w.rec_hi[j]] = [w.rec_hi[j], w.rec_hi[i]];
        [w.dec_lo[i], w.dec_lo[j]] = [w.dec_lo[j], w.dec_lo[i]];
        [w.dec_hi[i], w.dec_hi[j]] = [w.dec_hi[j], w.dec_hi[i]];
      }
    ...
    case 'BIOR':
      ...
      let N = Math.floor(order / 10), M = order % 10
      let M_idx
      let M_max
